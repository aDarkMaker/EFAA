...
config = {
    ...
    'version': version, #版本
    'my_app': ['resource.globals', 'Globals'], #可选. 全局单例对象, 可以存放加载的模型, 使用og.my_app调用
    'onetime_tasks': [  # 用户点击触发的任务
        ["resource.tasks.MyOneTimeTask", "MyOneTimeTask"],
        ["resource.tasks.MyOneTimeWithAGroup", "MyOneTimeWithAGroup"],
        ["resource.tasks.MyOneTimeWithAGroup2", "MyOneTimeWithAGroup2"],
        ["resource.tasks.MyOneTimeWithBGroup", "MyOneTimeWithBGroup"],
        ["ok", "DiagnosisTask"],
    ],
    'trigger_tasks':[ # 不断执行的触发式任务
        ["resource.tasks.MyTriggerTask", "MyTriggerTask"],
    ],
    'custom_tabs': [
        ['resource.ui.MyTab', 'MyTab'], #可选, 自定义UI, 显示在侧边栏
    ],
}